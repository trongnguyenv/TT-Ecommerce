namespace TTEcommerce.Core.Infrastructure.Http;
...
public class HttpRequester : IHttpRequester
{
    ...
    public async Task<T> PutAsync<T>(string url, object? body = null, string? bearerToken = null) where T : class
    {
        SetAuthorizationHeader(bearerToken);
        HttpResponseMessage responseMessage;

        if (body is null)
            responseMessage = await _httpClient.PutAsync(url, null);
        else
