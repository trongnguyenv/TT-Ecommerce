...
public class DebeziumConnectorSetup : IDebeziumConnectorSetup
{
    ...
    public async Task StartConfiguringAsync(CancellationToken cancellationToken = default)
    {
        ...
        await policyWrap.ExecuteAsync(async () =>
        {
            ...
            _logger.LogInformation("Configuring debezium with config: {debeziumConfigData}",
                nameof(debeziumConfigData));
            ...
            {
                _logger.LogError("Was not possible to configure Debezium. Status code: {statusCode}",
                    response.StatusCode);
